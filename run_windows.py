#!/usr/bin/env python3
"""
start_windows.py - Integrated Script for Windows
...
It performs the following steps:
  ...
  5. Uses Docker Compose to stop any running containers.
...
Potential failure conditions include missing dependencies, Docker not running, network issues during download, or Docker command failures.
"""
...
def check_docker_running():
    try:
        ...
        print("Error: Docker does not appear to be running. Please start Docker Desktop first.")
        sys.exit(1)
...
def docker_compose_down(root_dir):
    try:
        subprocess.check_call(["docker", "compose", "down"], cwd=root_dir)
    except subprocess.CalledProcessError:
...
def docker_compose_up(root_dir):
    subprocess.check_call(["docker", "compose", "up", "-d"], cwd=root_dir)
...
def main():
    ...
    if backend_ready and not frontend_ready:
        ...
        subprocess.call(["docker", "compose", "logs", "backend"])
        sys.exit(1)
