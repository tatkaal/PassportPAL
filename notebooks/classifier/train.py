...
def visualize_misclassified_samples(model, dataloader, class_names, device='cuda', num_samples=10):
    """
    Visualize samples that are misclassified by the model.
    ...
    if misclassified_images:
        ...
        for i, (img, true_label, pred_label) in enumerate(zip(misclassified_images, misclassified_true_labels, misclassified_pred_labels)):
            if i < len(axes):
                axes[i].imshow(img)
        ...
        print("No misclassified samples found in the current batch.")
